...
ggoutlier_compositeKNN <- function(geo_coord, gen_coord = NULL, pgdM = NULL,
                                           k_geneticKNN = NULL,
                                           k_geoKNN = NULL,
                                           klim = c(3,100),
                                           plot_dir = ".",
  ...
  if(is.null(k)){stop("k_geneticKNN is NULL!")}
  ...
  knn.indx <- find_gen_knn(pgdM, k=k_geneticKNN)
  ...
  if(is.null(k)){stop("k_geoKNN is NULL!")}
  ...
  ##---------------------------------------------------
  ##---------------------------------------------------
  knn.indx <- find_geo_knn(geo.dM = geo.dM, k=k_geoKNN, min_nn_dist=min_nn_dist)
  ...
  ##-------------------------------------------------------------------
  ##-------------------------------------------------------------------
  ## Get joint probability null distribution
  ##-------------------------------------------------------------------
  ##-------------------------------------------------------------------
  ### Define negative log likelihood function (sum up log(p1 * p2) -> log(p1) + log(p2))
  negLL <- function(a_geneticKNN, b_geneticKNN, a_geoKNN, b_geoKNN){
    -sum(dgamma(Dgeo, shape = a_geneticKNN,
                      rate = b_geneticKNN, log = T) +
           dgamma(Dg, shape = a_geoKNN,
                      rate = b_geoKNN, log = T))
  }
# CONTINUE FROM HERE
