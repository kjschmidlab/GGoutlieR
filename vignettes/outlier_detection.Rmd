...
* obtain a heuristic null distribution with maximum likelihood estimation.
* iteratively search KNNs, conduct statistical tests and remove the most significant samples until no new significant sample
...
The `method = "composite"` is recommended. 
...
geneticknn_res <- readRDS("../data/IPK_landrace_ggoutlier_geneticKNN_output_p0.01.RDS")
geoknn_res <- readRDS("../data/IPK_landrace_ggoutlier_geoKNN_output_p0.01.RDS")
compositeknn_res <- readRDS("../data/IPK_landrace_ggoutlier_composite_output_p0.01.RDS")
```

After running `ggoutlier`, we `summary_ggoutlier` to extract the IDs and p-values of outliers. 
