...
TO DO:
 ...
 - add scalar and w to output
--->

## Introduction

This is a short tutorial of the R package `GGoutlieR`.
...
It will produce a `list` object recording all details, including statistics, p-values, the IDs of nearest neighbors of every samples, and etc.
...
For visualization, `plot_ggoutlier` takes the outputs from `ggoutleir` to overlay colored lines connecting outliers to its K nearest neighbors.
The depth of color and the width of lines are proportional to the -log(p) values.
...
* 1,661 geo-referenced landraces (no duplicates)
      * 29,219 SNPs; LD prunned with SNP $r^2$ < 0.1; MAF > 0.01; no missing   
      
The `vcf.gz` file is available at: ADD_GITLABorGITHUB_LINK_HERE/`IPKbarley_GBS1661landraces_LDpruned.vcf.gz`
...
```{r get_numeric_genotypic_matrix}
library(vcfR)
...
```{r put_barley_data_in_package, eval=FALSE}
...
The IPK barley landrace data are available in `GGoutlieR` package.
...
(Please find the details of `GGoutlieR` framework in the supplementary material: ADD_GITLAB_LINK_HERE)

In short, `GGoutlieR` framework include following major steps:

* search for an optimal number of nearest neighbors which is the default of `ggoutlier` function. Users can specify number of nearest neighbors with the argument `K`.   
...
compositeknn_res <- readRDS("../data/IPK_landrace_ggoutlier_composite_output_p0.025.RDS")
```
...
To visualize outliers on a geographical map, `plot_ggoutlier` draws colored lines connecting outliers to corresponding nearst neighbors on a geographical map.
The color depth and thickness of lines are proportional to the -log(p) values.

As described in the supplementary material (ADD_GITLAB_LINK), `GGoutlieR` framework identifies outliers based on two types of KNN approaches:

* **geographical KNN**: K nearest neighbors defined based on geographical distances which are used to predict genetic components.  
* **genetic KNN**: K nearest neighbors defined based on genetic distances which are used to predict geographical locations.

In the graph made by `plot_ggoutlier`, the geographical KNNs of outliers are displayed with blue lines, whereas the genetic KNNs are presented with red lines.
...
The code block below produced two figures
...
*Ancestry coefficients and geo-genetic associations of outliers identified by GGoutlieR*

`plot_ggoutlier` can crop the network graph by setting `select_xlim` and `select_ylim`.
The codes below retain the links between outliers located in UK and corresponding KNNs.
We can find that the outliers in UK are genetically close to the samples in Nepal.
This pattern would be difficult to identify with conventional visualization tools, such as PCA.
...
```{r make_fig_for_paper,eval=FALSE,echo=FALSE}
